...
| Threat | Control |
| --- | --- |
| Command injection | The UI never accepts a command or executable path. `Process` uses fixed arguments and an absolute executable found only in absolute `PATH` entries. |
